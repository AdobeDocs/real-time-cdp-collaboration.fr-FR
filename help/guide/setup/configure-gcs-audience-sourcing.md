...
>[!NOTE]
>
>Un guide dédié couvrant les rôles IAM [!DNL Google Cloud] spécifiques, la configuration du compte de service et les autorisations au niveau du compartiment requises pour cette intégration est en attente de publication. Tant que ce guide n’est pas disponible, contactez votre administrateur [!DNL Google Cloud] pour vérifier qu’Adobe dispose des autorisations requises pour s’authentifier dans votre compartiment et lire les fichiers d’audience.

Avant de poursuivre, vérifiez les points suivants auprès de votre administrateur [!DNL Google Cloud] :

* Adobe a reçu les autorisations requises pour s’authentifier par rapport à votre compartiment GCS et lire les fichiers d’audience.
...
Vos fichiers d’audience doivent être conformes à la **[Spécification d’approvisionnement de l’audience (v1.2)](../../assets/quick-start/RTCDP_Collaboration_Audience_Sourcing_Spec_v1.2.pdf)** avant que l’approvisionnement ne commence. Consultez la spécification pour la définition de schéma complète et les exemples au niveau du champ. Les principales exigences sont les suivantes :
...
Une boîte de dialogue prérequise décrivant les étapes de configuration requises (par exemple, la configuration du compartiment GCS et l’affectation de rôle IAM) s’affiche et indique que les données doivent être conformes à la **[[!UICONTROL Spécification d’approvisionnement d’audience]](../../assets/quick-start/RTCDP_Collaboration_Audience_Sourcing_Spec_v1.2.pdf)**. Sélectionnez **[!UICONTROL Démarrer l’intégration]** pour confirmer la conformité avant de continuer.
...
Vérifiez que les mappages affichés reflètent les champs de vos fichiers d’audience. Dans le cas contraire, arrêtez et corrigez vos fichiers pour qu’ils soient conformes à la [Spécification d’approvisionnement de l’audience](../../assets/quick-start/RTCDP_Collaboration_Audience_Sourcing_Spec_v1.2.pdf) avant de continuer. Sélectionnez **[!UICONTROL Suivant]** pour continuer.
...
* Vérifiez que les fichiers mis à jour dans l’intervalle sont conformes à la structure des colonnes et aux exigences des champs dans la [Spécification d’origine d’audience](../../assets/quick-start/RTCDP_Collaboration_Audience_Sourcing_Spec_v1.2.pdf).
